...
func TestReElection2A(t *testing.T) {
	...
	log.Println("2A reelects begin check leader is ", leader1)

	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	cfg.checkOneLeader()
	log.Println("2A reelects again check leader is ", cfg.checkOneLeader())

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	log.Println("2A reelects third check leader is ", cfg.checkOneLeader())
...
func TestRejoin2B(t *testing.T) {
	...
	log.SetPrefix(fmt.Sprintf("leader %d disconnected: ", leader1))

	log.Printf("leader %v is disconnect", leader1)
	...
	leader2 := cfg.checkOneLeader()
	log.SetPrefix(fmt.Sprintf("leader %d disconnected: ", leader2))
	cfg.disconnect(leader2)

	// old leader connected again
	cfg.connect(leader1)
	log.SetPrefix(fmt.Sprintf("leader %d back: ", leader1))

	cfg.one(104, 2, true)

	// all together now
	cfg.connect(leader2)
	log.SetPrefix(fmt.Sprintf("leader %d back: ", leader2))
